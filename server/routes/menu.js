...
module.exports = (db) => {
  const insertMenuIntoDB = function (menu_id, menuArray) {
    for (let meal of menuArray) {
      // console.log(meal)
      db.query(
        `INSERT INTO meals (menu_id, spoonacular_id, title, day, category, order_by) VALUES ($1, $2, $3, $4, $5, $6);`,
        [menu_id, meal.spoonacular_id, meal.title, meal.day, meal.meal, meal.id]
      );
    }
        ...
        db.query(
          ...
          .then((user) => {
            ...
            if (restrictionsString) {
              restrictionsString =
                "&exclude=" + restrictionsString.slice(0, -2);
            }
            ...
            axios
              .get(
                `https://api.spoonacular.com/mealplanner/generate?apiKey=${process.env.SPOONACULARAPIKEY}&timeFrame=week&targetCalories=${userDetails.caloric_target}${dietString}${restrictionsString}`,
                { params, headers }
              )
              .then(async (data) => {
              ...
              .then(async (menuArray) => {
                ...
                console.log("user ID: ", req.body.user_id);
                const menuID = await db.query(
                  `INSERT INTO menus (user_id) VALUES ($1) RETURNING id;`,
                  [req.body.user_id]
                );
  ...
  router.post("/replace_weekly_menu", (req, res) => {
    console.log("user ID: ", req.body.user_id);
    db.query(`DELETE FROM menus WHERE user_id = $1 RETURNING *;`, [
      req.body.user_id,
    ]).then((user) => {
  ...
  router.get("/meal/:id", (req, res) => {
    ...
    axios
      .get(
        `https://api.spoonacular.com/recipes/${req.params.id}/information?apiKey=${process.env.SPOONACULARAPIKEY}`,
        { headers }
      )
      .then(async (data) => {
